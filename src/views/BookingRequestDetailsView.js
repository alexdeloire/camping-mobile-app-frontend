...
const BookingRequestDetailsView = ({ route, navigation }) => {
  ...
  const putComment = async () => {
    try {
      ...
      reservationDetails.ratingHostStar = hostRating;
      reservationDetails.ratingHostComment = reviewComment;
  ...
  useEffect(() => {
    ...
    setHostRating(item.ratingHostStar);
    setReviewComment(item.ratingHostComment);
    setHasCommented(item.ratingHostStar && item.ratingHostStar > 0);
  }, []);
      ...
      {['PENDING', 'CONFIRMED'].includes(item.state) ? (
        ...
        <View style={styles.reviewSection}>
          {hasCommented ? (
            <>
              ...
              <View style={styles.starRating}>
                {[...Array(5)].map((_, i) => (
                  <Ionicons
                    key={i}
                    name={i < hostRating ? "star" : "star-outline"}
                    size={24}
                    color="green"
                  />
                ))}
            ...
            <>
              <Text style={styles.reviewTitle}>Leave a review for the client </Text>
              <View style={styles.starRating}>
                {[...Array(5)].map((_, i) => (
          ...
          <View style={styles.hostReview}>
            ...
            {(item.ratingClientStar && item.ratingClientStar > 0) ? (
              <>
                <View style={styles.starRating}>
                {[...Array(5)].map((_, i) => (
                  <Ionicons
                  key={i}
                  name={i < item.ratingClientStar ? "star" : "star-outline"} 
                  size={20}
                  color="green"
                  />
                ))}
              </View>
              <Text style={styles.hostComment}>{item.ratingClientComment || "No review from the client."}</Text>
            </>    
            ) : (
              <>
                <Text style={styles.hostComment}>No review from the client yet.</Text>
              </>
            )}
          </View>
        </View>
...
export default BookingRequestDetailsView;
